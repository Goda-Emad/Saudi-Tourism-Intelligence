...
C = {
    ...
    "gold":     "#C9A84C", "blue":     "#365C8D", "card_bg":  "#1E2528",
} if THEME == "dark" else {
    ...
    "gold":     "#A67C00", "blue":     "#365C8D", "card_bg":  "#FFFFFF",
}
...
"EN": {
    ...
    "thm_btn":"☀️ Light" if THEME=="dark" else "🌙 Dark",
    "lng_btn":"🌐 العربية",
},
"AR": {
    ...
    "hs":"تحليلات مدعومة بالذكاء الاصطناعي على 10 سنوات من البيانات الرسمية.\nتوقعات · تقسيم · استدامة — كل شيء في منصة واحدة.",
    ...
    "thm_btn":"☀️ فاتح" if THEME=="dark" else "🌙 داكن",
    "lng_btn":"🌐 English",
},
}
...
logo_img = ('<img src="' + logo_src + '" style="height:44px;border-radius:8px;"/>'
...
st.markdown(
    ...
    "padding:9px 0!important;margin-bottom:6px!important;"
    ...
    "[data-testid=stTickBarMin],[data-testid=stTickBarMax]{color:" + C["grey"] + "!important;}"
    # ── Selectbox ──
    "[data-baseweb=select]>div{"
    "background:" + C["card_bg"] + "!important;border-color:" + C["border"] + "!important;"
...
# ════════════════════════════════════════════════════════════════════
# SIDEBAR  (theme + lang buttons live here — no duplication)
# ════════════════════════════════════════════════════════════════════
with st.sidebar:
    # Logo + name
    st.markdown(
        '<div style="display:flex;align-items:center;gap:10px;padding:18px 4px 14px;">'
        + logo_img +
        '<div>'
    ...
    st.markdown('<div style="height:1px;background:' + C["border"] + ';margin-bottom:14px;"></div>', unsafe_allow_html=True)

    # Theme & Lang buttons
    if st.button(t["thm_btn"], key="k_thm", use_container_width=True):
        st.session_state.theme = "light" if THEME=="dark" else "dark"; st.rerun()
    if st.button(t["lng_btn"], key="k_lng", use_container_width=True):
        st.session_state.lang = "AR" if LANG=="EN" else "EN"; st.rerun()

    st.markdown('<div style="height:1px;background:' + C["border"] + ';margin:14px 0;"></div>', unsafe_allow_html=True)

    # Nav links
    pages_nav = [
        ("🏠","Executive Overview","pages/01_🏠_Overview.py"),
        ...
        ("🏠","النظرة التنفيذية","pages/01_🏠_Overview.py"),
    ...
    for ico, label, _ in pages_nav:
        st.markdown(
            '<div style="display:flex;align-items:center;gap:8px;'
            'padding:8px 10px;border-radius:7px;margin-bottom:3px;'
            'transition:background .2s;">'
            '<span>' + ico + '</span>'
            '<span style="font-size:.82rem;font-weight:500;color:' + C["grey"] + ';">' + label + '</span>'
            '</div>',
            unsafe_allow_html=True,
        )

    st.markdown('<div style="height:1px;background:' + C["border"] + ';margin:14px 0 10px;"></div>', unsafe_allow_html=True)
    st.markdown(
        '<div style="font-size:.68rem;color:' + C["grey"] + ';padding:0 4px;">'
...
st.markdown(
    '<div style="position:relative;width:100%;aspect-ratio:16/6;overflow:hidden;'
    ...
    'padding:4% 5%;max-width:50%;height:100%;'
    ...
    'border-radius:4px;margin-bottom:20px;width:fit-content;">'
    + t["pill"] + '</div>'

    # H1
    '<div style="font-size:clamp(2.2rem,4vw,3.6rem);font-weight:800;'
    'color:' + C["white"] + ';line-height:1.0;'
    'letter-spacing:-1px;margin-bottom:6px;">'
    + t["h1"] + '</div>'

    # H2 colored
    '<div style="font-size:clamp(2.2rem,4vw,3.6rem);font-weight:800;'
    'color:' + C["teal"] + ';line-height:1.0;'
    'letter-spacing:-1px;margin-bottom:22px;">'
    + t["h2"] + '</div>'

    # subtitle
    '<p style="font-size:clamp(.8rem,1.1vw,.95rem);'
    'color:' + C["grey"] + ';line-height:1.8;'
    ...
    'border-radius:7px;text-decoration:none;width:fit-content;'
    'box-shadow:0 6px 28px ' + C["teal"] + '55;'
...
for i, (val, lbl, ck) in enumerate(t["stats"]):
    ...
    cells += (
        '<div style="padding:28px 24px;' + br + 'transition:background .2s;">'
        '<div style="font-size:2rem;font-weight:700;color:' + clr(ck) + ';'
        'font-family:IBM Plex Mono,monospace;letter-spacing:-1px;">' + val + '</div>'
        '<div style="font-size:.64rem;color:' + C["grey"] + ';'
        'text-transform:uppercase;letter-spacing:1px;'
        'font-weight:500;margin-top:6px;">' + lbl + '</div>'
        '</div>'
    )
st.markdown(
...
def sec_head(badge_lbl, h2, sub=""):
    out = (
        ...
        'margin-bottom:12px;">' + badge_lbl + '</div>'
        '<div style="font-size:1.5rem;font-weight:700;color:' + C["white"] + ';'
        'margin-bottom:6px;">' + h2 + '</div>'
    )
    if sub:
        out += '<div style="font-size:.82rem;color:' + C["grey"] + ';">' + sub + '</div>'
    return out + '</div>'
