...
C = {
    ...
    # ✅ FIX: full light-mode palette corrected (was white=#F4F9F8, grey=#9DBFBA, border=#2A3235)
...
"EN":{
    ...
    "spend_ratio":"Inbound vs Domestic (2024)",
    ...
    "year_range":"Year Range",   # ✅ FIX: was hardcoded in slider
    "insight_title":"Key Insights",
    # ✅ FIX: i3 corrected — 13,053 not in data; best year is 2022 SAR 5,907
    "i1":"Inbound tourists spend 4.0× more than Domestic per trip (SAR 5,007 avg vs SAR 1,242 avg)",
    "i2":"Religious purpose generates the highest total expenditure — 391.2 Billion SAR",
    "i3":"Best single year: 2022 — SAR 5,907 per Inbound trip (+70% vs COVID low of SAR 3,469)",
    "i4":"Inbound spending collapsed -35% in 2020 (COVID) then surged to SAR 5,907 in 2022",
    "growth_label":"Growth 2015–2024","covid_low":"COVID Low (2020)","peak_spend":"Peak (2022)",
},
"AR":{
    ...
    "spend_ratio":"نسبة الوافد للمحلي (2024)",
    ...
    "i1":"الوافدون ينفقون 4.0× أضعاف المحليين لكل رحلة (5,007 مقابل 1,242 ريال متوسط)",
    "i2":"الغرض الديني يولد أعلى إنفاق إجمالي — 391.2 مليار ريال",
    "i3":"أفضل سنة: 2022 — 5,907 ريال لكل رحلة وافدة (+70% من أدنى مستوى كوفيد 3,469 ريال)",
    "i4":"إنفاق الوافدين انهار -35% في 2020 ثم قفز لـ 5,907 ريال في 2022",
    "growth_label":"نمو 2015–2024","covid_low":"أدنى كوفيد (2020)","peak_spend":"الذروة (2022)",
},
}
t = TR[LANG]
...
exp_inbound = [391.20,123.60,71.10,34.52,29.09]   # sum = 649.51B
...
def apply_layout(fig, height=340):
    fig.update_layout(
        ...
        # ✅ FIX: linecolor theme-aware via C["border"] (was hardcoded #2A3235)
...
# ════════════════════════════════════════════════════════════════════
# KPI STRIP
# ✅ FIX: ratio KPI updated to 4.2× (5622/1336) with correct denominator label
# ════════════════════════════════════════════════════════════════════
st.markdown(
    f'<div style="padding:20px 40px 0;">'
    f'<div style="display:grid;grid-template-columns:repeat(5,1fr);gap:12px;">'
    +kpi_card("✈️💰",t["avg_trip_in"],  "SAR 5,007","10-yr avg",  "gold")
    +kpi_card("🏠💰",t["avg_trip_dom"], "SAR 1,242","10-yr avg",  "teal")
    +kpi_card("🌙💰",t["avg_night_in"], "SAR 482",  "2024",       "blue")
    +kpi_card("🏡💰",t["avg_night_dom"],"SAR 234",  "2024",       "purple")
    +kpi_card("⚖️",  t["spend_ratio"],  "4.2×",     "5,622/1,336","red")
    +'</div></div>',
    unsafe_allow_html=True)
...
with yc:
    # ✅ FIX: slider label now uses t["year_range"] (bilingual)
    year_range = st.slider(
        t["year_range"],min_value=2015,max_value=2024,
...
# ✅ FIX: define show_inb/show_dom once (was repeated inline 4 times)
show_inb = tourist_filter in [t["all_types"],t["inbound"]]
show_dom = tourist_filter in [t["all_types"],t["domestic"]]
...
with monthly_col:
    ...
    fig_monthly.update_layout(xaxis=dict(tickfont=dict(size=9),tickangle=45))
...
def cmp_row(lbl,val,color):
    return (f'<div style="display:flex;justify-content:space-between;align-items:center;'
            ...
            f'<span style="font-size:.82rem;color:{txt_col};font-weight:500;">{lbl}</span>'
...
with cmp_col3:
    ...
    # ✅ FIX: annotation corrected to 649.5B (was 649B — actual sum=649.51B)
    fig_donut.add_annotation(
        text=f"<b>649.5B</b><br>Inbound Total",x=0.5,y=0.5,
        showarrow=False,font=dict(size=11,color=txt_col))
    ...
    fig_donut.update_layout(
        ...
        title=dict(text="Inbound Expenditure Split",
                   font=dict(size=11,color=txt_col),x=0.5))
    chart(fig_donut)
...
# ════════════════════════════════════════════════════════════════════
# FOOTER  ✅ FIX: hardcoded #B5B8B7 replaced with C["foot_txt"]
# ════════════════════════════════════════════════════════════════════
st.markdown(
    ...
    f'<div style="font-size:.66rem;color:{C["foot_txt"]};margin-top:2px;">💰 Spending · Eng. Goda Emad</div>'
    f'</div></div>'
