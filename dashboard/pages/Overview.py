...
"EN":{
    ...
    # ✅ FIX: YoY corrected — Total +13.1%, Nights +18.5%, Avg Stay +23%
    "k":[
        ("115.8M","Total Tourists 2024",   "teal",  "▲ +13.1%"),
        ("30.1M", "Inbound Tourists",      "teal",  "▲ +9.9%"),
        ("85.7M", "Domestic Tourists",     "blue",  "▲ +14.3%"),
        ("1.10B", "Overnight Stays",       "teal",  "▲ +18.5%"),
...
"AR":{
    ...
    "k":[
        ...
        ("30.1M", "السياح الوافدون",       "teal",  "▲ +9.9%"),
        ("85.7M", "السياح المحليون",       "blue",  "▲ +14.3%"),
        ("1.10B", "ليالي الإقامة",        "teal",  "▲ +18.5%"),
...
RECOVERY = [round(v/TOTAL[4]*100,1) for v in TOTAL]  # base = 2019 (index 4)

# ✅ FIX: bilingual months
MONTHS_EN = ["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"]
MONTHS_AR = ["يناير","فبراير","مارس","أبريل","مايو","يونيو","يوليو","أغسطس","سبتمبر","أكتوبر","نوفمبر","ديسمبر"]
MONTHS    = MONTHS_AR if LANG=="AR" else MONTHS_EN

# ✅ FIX: monthly values scaled to sum to 115.8M (2024 actual)
...
def apply_layout(fig, height=360):
    fig.update_layout(
        ...
        # ✅ FIX: linecolor now C["border"] — theme-aware (was hardcoded #2A3235)
...
# ✅ FIX: added padding wrapper (was missing)
st.markdown('<div style="padding:0 40px;">',unsafe_allow_html=True)
c1,c2 = st.columns([3,2],gap="large")

with c1:
    fig1 = go.Figure()
    fig1.add_trace(go.Bar(x=YEARS,y=DOMESTIC,name=t["dom"],
        marker=dict(color=C["blue"],opacity=.85,line=dict(width=0)),
        hovertemplate="%{x}: <b>%{y}M</b><extra></extra>"))
    fig1.add_trace(go.Bar(x=YEARS,y=INBOUND,name=t["inb"],
        marker=dict(color=C["teal"],opacity=.85,line=dict(width=0)),
        hovertemplate="%{x}: <b>%{y}M</b><extra></extra>"))
    fig1.add_trace(go.Scatter(x=YEARS,y=TOTAL,name=t["tot"],
...
with c2:
    ...
    # ✅ FIX: added 2024 recovery annotation
...
with c3:
    ...
    fig3 = go.Figure(go.Pie(
        ...
        textfont=dict(size=11,color="#F4F9F8"),   # always light on colored slices
        hovertemplate="<b>%{label}</b>: %{value}% (%{percent})<extra></extra>"))
    fig3.add_annotation(text="2024",x=.5,y=.56,showarrow=False,
                        font=dict(size=11,color=C["grey"]))
    fig3.add_annotation(text="Purpose" if LANG=="EN" else "الغرض",
...
with c4:
    ...
    fig4 = go.Figure(go.Bar(
        ...
        text=[f"{v}%" for v in NAT_V],textposition="outside",
...
fig5.add_trace(go.Scatter(
    ...
    mode="lines+markers",
    marker=dict(size=8,
                color=[C["orange"] if v==max(MONTHLY) else C["teal"] for v in MONTHLY],
                line=dict(width=1.5,color=C["navbar"])),
    hovertemplate="<b>%{x}</b>: %{y:.2f}M tourists<extra></extra>"))

# ✅ FIX: vrect x-values now use MONTHS list (bilingual-aware)
fig5.add_vrect(x0=MONTHS[1],x1=MONTHS[2],
    fillcolor="rgba(196,154,76,0.12)",line_width=0,
    annotation_text=t["ramadan"],
    annotation_font=dict(color=C["gold"],size=10))
fig5.add_vrect(x0=MONTHS[5],x1=MONTHS[7],
    fillcolor="rgba(23,177,155,0.08)",line_width=0,
...
for ico,txt_i,ck in t["ins"]:
    ins_html += (
        ...
        f'<div style="font-size:.83rem;color:{C["white"]};line-height:1.65;">{txt_i}</div>'
        f'</div>')
ins_html += '</div></div>'
st.markdown(ins_html,unsafe_allow_html=True)

# ════════════════════════════════════════════════════════════════════
# FOOTER  ✅ FIX: was completely missing
# ════════════════════════════════════════════════════════════════════
st.markdown(
