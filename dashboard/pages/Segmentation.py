...
C = {
    ...
    # ✅ FIX: full light-mode palette corrected
...
"EN":{
    ...
    "k_choice":"K Selection (Silhouette Analysis)",
    ...
    "i2":"Budget segment (Domestic) is 3× larger but generates 4× less revenue per trip",
    "i3":"Inbound transitioned from Mid-Value to High-Value segment post-pandemic",
    "i4":"Targeting High-Value segment = 4.4× more spend per trip vs Budget segment",
    ...
    "k3_note":"k=3 chosen for interpretability — 3 natural tourist types",
},
"AR":{
    ...
    "k_choice":"اختيار K (تحليل Silhouette)",
    ...
    "i2":"الشريحة الاقتصادية (المحلية) أكبر 3× لكنها تولد إيرادات أقل 4× لكل رحلة",
    "i3":"الوافدون انتقلوا من متوسط القيمة إلى عالي القيمة بعد الجائحة",
    "i4":"استهداف شريحة عالي القيمة = إنفاق أكثر 4.4× لكل رحلة مقارنة بالاقتصادي",
    ...
    "k3_note":"تم اختيار k=3 للقابلية للتفسير — 3 أنواع طبيعية للسياح",
},
}
t = TR[LANG]
...
segments = {
    "high":{
        ...
        "tourists":24514,
        "avg_trip":5512,
        # ✅ FIX: avg_night = avg_trip / avg_los = 5512/17.1 = 322 SAR/night
        "avg_night":322,
        "avg_los":17.1,"type":"Inbound","years":"2022–2024","revenue_index":100,
    },
    "mid":{
        "name":t["seg_mid"],"color":accent_blue,"emoji":"🌟",
        "tourists":13232,
        "avg_trip":4791,
        # ✅ FIX: avg_night = 4791/9.2 = 521 SAR/night
        "avg_night":521,
        "avg_los":9.2,"type":"Inbound","years":"2015–2021","revenue_index":87,
    },
    "budget":{
        "name":t["seg_budget"],"color":accent_teal,"emoji":"🏠",
        "tourists":57822,
        "avg_trip":1242,
        # ✅ FIX: avg_night = 1242/5.4 = 230 SAR/night
        "avg_night":230,
        "avg_los":5.4,"type":"Domestic","years":"2015–2024","revenue_index":23,
    },
}
...
# ✅ FIX: timeline arrays corrected to match annual inbound/domestic totals
inbound_total  = [17500,18000,16100,15300,14100,6300,11500,16000,27400,30100]
domestic_total = [68200,72000,74000,77000,80500,40000,55000,62000,75000,85700]
...
st.markdown(
    ...
    f".seg-row{{display:flex;justify-content:space-between;padding:6px 0;"
    f"border-bottom:1px solid {C['border']};}}"
    f".seg-row:last-child{{border-bottom:none;}}"
    "</style>",
...
def apply_layout(fig, height=340):
    fig.update_layout(
        ...
        # ✅ FIX: linecolor uses C["border"] — theme-aware (was hardcoded #2A3235)
...
badge_style = (f'display:inline-block;background:{C["sec_bg"]};border:1px solid {C["border"]};'
               f'border-radius:20px;padding:5px 14px;font-size:.78rem;color:{txt_col};'
...
st.markdown(
    ...
    +kpi_card("⚡","Revenue Ratio",     "4.4×",      "Spend per trip","purple")
    +'</div></div>',
    unsafe_allow_html=True)
...
with scatter_col:
    fig_s = go.Figure()
    seg_c = {"high":accent_gold,"mid":accent_blue,"budget":accent_teal}
    seg_n = {"high":t["seg_high"],"mid":t["seg_mid"],"budget":t["seg_budget"]}
    for key,pts in scatter_data.items():
        fig_s.add_trace(go.Scatter(
            x=[p[0] for p in pts],y=[p[1] for p in pts],
            mode='markers',name=seg_n[key],
            marker=dict(color=seg_c[key],size=10,opacity=0.75,
                        line=dict(color=bg_card,width=1.5)),
            hovertemplate=f"<b>{seg_n[key]}</b><br>LOS: %{{x:.1f}} nights<br>Spend: SAR %{{y:,.0f}}<extra></extra>"))
    centroids = [
...
with radar_col:
    ...
    mid_vals    = [100*13232/57822,  87,   100*521/322, 100*9.2/17.1, 87]
    budget_vals = [100,  100*1242/5512, 100*230/322, 100*5.4/17.1, 23]

    fig_r = go.Figure()
    for nm,vals,color in [
...
with size_col:
    ...
    fig_p = go.Figure(go.Pie(
        ...
        values=[24514,13232,57822],hole=0.55,
    ...
    fig_p.add_annotation(text="<b>95.6M</b><br>Total",x=0.5,y=0.5,
        showarrow=False,font=dict(size=12,color=txt_col))
    apply_layout(fig_p,height=280)
...
with sil_col:
    ...
    # ✅ FIX: explain why k=3 chosen over k=4 (interpretability)
    fig_sil.add_annotation(x="k=4",y=0.644,
        text="*interpretability",showarrow=False,
        font=dict(size=8,color=C["grey"]),yshift=28)
    ...
    chart(fig_sil)
    # ✅ FIX: add note about k=3 rationale
    st.markdown(
        f'<div style="font-size:.72rem;color:{C["grey"]};margin-top:4px;'
        f'padding:8px;background:{C["sec_bg"]};border-radius:6px;">'
        f'* {t["k3_note"]}</div>',
        unsafe_allow_html=True)
...
st.markdown(
    ...
    # ✅ FIX: footer text uses C["foot_txt"] (theme-aware)
    f'<div style="font-size:.66rem;color:{C["foot_txt"]};margin-top:2px;">🎯 Segmentation · Eng. Goda Emad</div>'
    f'</div></div>'
